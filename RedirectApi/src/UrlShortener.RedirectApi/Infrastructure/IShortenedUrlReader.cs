...
public sealed class CosmosShortenedUrlReader(Container container) : IShortenedUrlReader
{
    public async Task<ReadLongUrlResponse> GetLongUrlAsync(string shortUrl, CancellationToken cancellationToken)
    {
        try
        {
            var record = await container.ReadItemAsync<CosmosUrlRecord>(
                shortUrl, new PartitionKey(shortUrl[..1]),
    ...
    sealed record CosmosUrlRecord(string LongUrl);
}